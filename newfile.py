...
import datetime
import time
token = '7200712641:AAEKaK5GQiV3t2ePrP_2kzlkP8pbsT1wYFk'
...
def start(message):
	bot.reply_to(message,'''𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐓𝐨 𝐎𝐦𝐚𝐫 𝐒𝐂𝐑𝐀𝐏 𝐁𝐨𝐭
𝙴𝚗𝚝𝚎𝚛 𝚢𝚘𝚞𝚛 𝚌𝚘𝚖𝚋𝚘 𝚜𝚘 𝚒𝚝 𝚌𝚊𝚗 𝚋𝚎 𝚌𝚑𝚎𝚌𝚔𝚎𝚍	
...
def main(message):
	...
	live = 0
	ch = 0
	ko = (bot.reply_to(message,"𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃 𝚆𝙷𝙸𝙻𝙴 𝚈𝙾𝚄𝚁 𝙲𝙰𝚁𝙳𝚂 𝙰𝚁𝙴 𝙱𝙴𝙸𝙽𝙶 𝙲𝙷𝙴𝙲𝙺 𝙰𝚃 𝚃𝙷𝙴 𝙶𝙰𝚃𝙴𝚆𝙰𝚈 stripe charge 𝙱𝚈  @u9p_l").message_id)
	...
	try:
		with open("combo-sjad.txt", 'r') as file:
			...
			for cc in lino:
				...
				bot.edit_message_text(chat_id=message.chat.id, message_id=ko, text='''𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃 𝚆𝙷𝙸𝙻𝙴 𝚈𝙾𝚄𝚁 𝙲𝙰𝚁𝙳𝚂 𝙰𝚁𝙴 𝙱𝙴𝙸𝙽𝙶 𝙲𝙷𝙴𝙲𝙺 𝙰𝚃 𝚃𝙷𝙴 𝙶𝙰𝚃𝙴𝚆𝙰𝚈 stripe charge 𝙱𝙾𝚃𝙱𝚈  @u9p_l ''', reply_markup=mes)
				end_time = time.time()
